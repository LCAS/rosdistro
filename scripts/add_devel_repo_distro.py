...
def add_devel_repository(yaml_file, name, vcs_type, url, version=None, status='developed'):
    ...
    if data['type'] != 'distribution':
        raise RuntimeError('The passed .yaml file is neither of type "source" nor "gbp", but type %s' % data['type'])
